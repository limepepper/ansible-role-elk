...
def install_plugin(module, plugin_bin, plugin_name, version, url, proxy_host, proxy_port, timeout):
    ...
    if proxy_host and proxy_port:
        cmd_args.append("-DproxyHost=%s -DproxyPort=%s" %
    ...
    if rc != 0:
        ...
        module.fail_json(msg='Is %s a valid plugin name?' %
                         plugin_name, err=reason)

    return True, cmd, out, err


def remove_plugin(module, plugin_bin, plugin_name):
    cmd_args = [plugin_bin, PACKAGE_STATE_MAP["absent"],
                parse_plugin_repo(plugin_name)]
...
def get_plugin_bin(module, plugin_bin=None):
    ...
    if plugin_bin and os.path.isfile(plugin_bin):
        ...
        for bin_file in plugin_bins:
            valid_plugin_bin = module.get_bin_path(
                bin_file, opt_dirs=plugin_dirs)
            if valid_plugin_bin:
                break
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            plugin_dir=dict(
                default="/usr/share/elasticsearch/plugins/", type="path"),
    ...
    if state == "present":
        changed, cmd, out, err = install_plugin(
            module, plugin_bin, name, version, url, proxy_host, proxy_port, timeout)

    elif state == "absent":
        changed, cmd, out, err = remove_plugin(module, plugin_bin, name)

    module.exit_json(changed=changed, cmd=cmd, name=name, state=state,
                     url=url, timeout=timeout, stdout=out, stderr=err)
